...
{
	if (code != nullptr) *code = init(filepath, createmode);
}

//Same as in N2ST
ir::ec ir::S2STDatabase::init(const schar *filepath, create_mode createmode) noexcept
{
	finalize();
...
void ir::S2STDatabase::finalize() noexcept
{
	_mapping.close();
